...
export default function Remboursements() {
  ...
  const [pendingAction, setPendingAction] = useState(null); // id du remb à valider/refuser
  ...
  const soumettreDemande = () => {
    ...
    // Also add to demandes context so admin sees it on Dashboard
    ajouterDemande({
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `livrr-remboursements-${new Date().toLocaleDateString("fr-FR").replace(/\//g,"-")}.csv`;
    a.click(); toast.success("Export CSV téléchargé");
  };
  ...
  return (
    <div className="page" style={{ padding: "32px 36px" }}>
      ...
      <div style={{ background: "#fff", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-sm)", border: "1px solid var(--white-3)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
          ...
          <tbody>
            {filtres.map((r) => {
              ...
              return (
                <tr key={r.id} onClick={() => setSelected(isSelected ? null : r.id)} style={{ borderBottom: "1px solid var(--white-3)", cursor: "pointer", background: isSelected ? "rgba(201,169,110,0.04)" : isEnAttenteAdmin ? "rgba(245,158,11,0.03)" : "transparent", transition: "background 0.15s" }}>
                  ...
                  <td style={{ padding: "11px 12px", fontSize: "12px", color: "var(--gray)", maxWidth: "180px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.declencheur}</td>
                  ...
                  <td style={{ padding: "11px 12px" }}>
                    <div style={{ display: "flex", gap: "4px" }}>
                      {/* Admin valide ou refuse les demandes SAV */}
                      {estAdmin && isEnAttenteAdmin && (
                        <>
                          <button onClick={(e) => { e.stopPropagation(); validerDemande(r.id); }} style={{ padding: "4px 10px", borderRadius: "var(--radius-sm)", fontSize: "11px", fontWeight: "600", background: "#e8f5ee", color: "#2e8b57", border: "1px solid #bbf7d0", cursor: "pointer" }}>
                            ✓ Valider
                          </button>
                          <button onClick={(e) => { e.stopPropagation(); setPendingAction(r.id); setShowRefusModal(true); }} style={{ padding: "4px 10px", borderRadius: "var(--radius-sm)", fontSize: "11px", fontWeight: "600", background: "#fef2f2", color: "#c0392b", border: "1px solid #fecaca", cursor: "pointer" }}>
                            Refuser
                          </button>
                        </>
                      )}
                      {/* Confirmer virement pour les remb en cours */}
                      {estAdmin && (r.statut === "en_cours" || r.statut === "en_attente") && (
                        <button onClick={(e) => { e.stopPropagation(); setSelected(r.id); setShowConfirmModal(true); }} style={{ padding: "4px 10px", borderRadius: "var(--radius-sm)", fontSize: "11px", fontWeight: "600", background: "#eff6ff", color: "#185fa5", border: "1px solid #bfdbfe", cursor: "pointer" }}>
                          Confirmer virement
                        </button>
                      )}
                    </div>
        ...
        {/* Détail motif SAV si sélectionné */}
        {remb?.motifSAV && (
          <div style={{ padding: "16px 20px", borderTop: "2px solid var(--white-3)", background: "#fafafa" }}>
            <div style={{ fontSize: "10px", fontWeight: "700", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--gray)", marginBottom: "8px" }}>Motif SAV — {remb.id}</div>
            <div style={{ fontSize: "13px", color: "var(--noir)", lineHeight: 1.7, fontStyle: "italic" }}>« {remb.motifSAV} »</div>
            <div style={{ fontSize: "11px", color: "var(--gray)", marginTop: "6px" }}>Demandé par <strong>{remb.demandePar}</strong> le {remb.dateCreation} à {remb.heureCreation}</div>
          </div>
        )}
