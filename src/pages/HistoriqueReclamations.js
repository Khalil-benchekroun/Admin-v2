...
export default function HistoriqueReclamations() {
  ...
  const stats = {
    ...
    résolus: HISTORIQUE_DATA.filter((r) =>
      [
        ...
        "retour",
        "geste_commercial",
      ].includes(r.resolution)
    ).length,
    montantTotal: HISTORIQUE_DATA.filter((r) => r.montant).reduce(
      (s, r) => s + r.montant,
      0
    ),
    tempsGlobal: "2h18",
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `livrr-reclamations-${new Date()
    ...
    <div className="page" style={{ padding: "32px 36px" }}>
      ...
      >
        <div>
          ...
          >
            Journal complet de toutes les réclamations — ouvertes, résolues et
            escaladées
          </p>
        </div>
        <button
      ...
      >
        {[
          ...
          { label: "Résolues", val: stats.résolus, color: "#2e8b57" },
          {
            ...
            val: stats.tauxResolution,
            color: "#2e8b57",
          },
          {
            label: "Temps moyen résol.",
            val: stats.tempsGlobal,
            color: "#185fa5",
          },
          {
      ...
      >
        <input
          type="text"
          placeholder="ID, client, boutique, sujet…"
          value={search}
        ...
        <button
          ...
          style={fBtn(filterRes === "all")}
        >
          Tous statuts
        </button>
        <button
          onClick={() => setFilterRes("résolu")}
          style={fBtn(filterRes === "résolu", "#2e8b57", "#e8f5ee")}
        >
          ✓ Résolu
        </button>
        <button
          onClick={() => setFilterRes("en_cours")}
          style={fBtn(filterRes === "en_cours", "#185fa5", "#eff6ff")}
        >
          En cours
        </button>
        <button
          onClick={() => setFilterRes("escalade")}
          style={fBtn(filterRes === "escalade", "#c0392b", "#fef2f2")}
        >
          Escaladé
        </button>
        <span
        ...
        >
          {filtres.length} réclamation{filtres.length > 1 ? "s" : ""}
        </span>
      </div>
          ...
          >
            ...
            <tbody>
              {filtres.map((r) => {
                ...
                const rc =
                  RESOLUTION_CFG[r.resolution] || RESOLUTION_CFG["résolu"];
                  ...
                  >
                    ...
                    <td
                      style={{
                        padding: "10px 12px",
                        fontSize: "11px",
                        color: r.tempResolution
                          ? "#2e8b57"
                          : "var(--gray-light)",
                        fontWeight: r.tempResolution ? "600" : "400",
                        whiteSpace: "nowrap",
                      }}
                    >
                      {r.tempResolution || "—"}
